...
export const Cover = ({ children, className }) => {
    ...
    return (
        <div
            ...
            ref={ref}
            className={`relative hover:bg-neutral-900 group inline-block dark:bg-neutral-900 bg-neutral-100 px-2 py-2 transition duration-200 rounded-sm ${className} `}
        >
            <AnimatePresence>
                {hovered && (
                    <motion.div
                        ...
                        className="h-full w-full overflow-hidden absolute inset-0 text-[#d79921] "
                    >
                        <motion.div
            ...
            {beamPositions.map((position, index) => (
                <Beam key={index} hovered={hovered} duration={Math.random() * 2 + 1} delay={Math.random() * 2 + 1} width={containerWidth} style={{ top: `${position}px` }} />
            ))}
            ...
            <motion.span
                ...
                className={`dark:text-[#d79921] inline-block text-neutral-900 relative z-20 group-hover:text-[#d79921]  transition duration-200 ${className} `}
            >
                {children}
            </motion.span>
...
export const CircleIcon = ({ className, delay }) => {
    return (
        <div
            className={`pointer-events-none animate-pulse group-hover:hidden group-hover:opacity-100 h-2 w-2 rounded-full bg-neutral-600 dark:bg-white opacity-20 group-hover:bg-white ${className}`}
        ></div>
    );
};
